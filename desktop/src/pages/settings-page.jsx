    ...
    <>
      ...
      <div className="settings-layout">
        ...
        <section className="settings-section" id="extension-settings" tabIndex="-1"><div className="settings-section-header section-header"><div><CardTitle>浏览器 Extension</CardTitle><CardDescription>Full Package 会预置 Extension；Core Package 请从 GitHub 下载后按浏览器指南加载。</CardDescription></div><Button variant="ghost" size="icon" aria-label="刷新 Extension 状态" onClick={refreshExtension} disabled={extensionBusy}><Icon name="refresh" size={18} /></Button></div><div className="settings-section-content">
          ...
          <div className="extension-websocket-status"><div><span>WebSocket 端口</span><strong>{extension.websocket_port || "未启动"}</strong></div><div><span>认证状态</span><strong>{extension.websocket_authenticated ? "已认证" : "未认证"}</strong></div><div><span>Desktop 观察</span><strong>{extension.websocket_connection || "未启动"}</strong></div>{extension.websocket_error && <p className="settings-message settings-message-error" role="status">WebSocket：{extension.websocket_error}</p>}{extension.websocket_token && <CopyablePath label="Extension 配对 token（复制到 Extension 设置）" value={extension.websocket_token} copied={copied === "websocket-token"} onCopy={() => copyPath("websocket-token", extension.websocket_token)} />}</div>

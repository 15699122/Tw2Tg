...
export default function SettingsPage({
  ...
  isWindows, aria2, aria2Busy, aria2CustomPath, setAria2CustomPath,
  ...
  setGalleryDlPath, saveGalleryDlPath, chooseGalleryDl, chooseAria2,
    ...
    <>
      ...
      <div className="settings-layout">
        <Card id="sidecar-settings" tabIndex="-1"><CardHeader><CardTitle>Sidecar 配置</CardTitle><CardDescription>Sidecar worker 负责 JSONL 协议；gallery-dl 负责实际媒体提取和下载。</CardDescription></CardHeader><CardContent>
          ...
          <CopyablePath label="gallery-dl 可执行文件" value={galleryDlPath} copied={copied === "sidecar"} onCopy={() => copyPath("sidecar", galleryDlPath)} />
          <div className="dependency-path-editor"><label htmlFor="gallery-dl-path">Core Package 外部 gallery-dl.exe 路径</label><div className="dependency-path-row"><input id="gallery-dl-path" value={galleryDlPath} onChange={(event) => setGalleryDlPath(event.target.value)} placeholder="例如 C:\\Tools\\gallery-dl\\gallery-dl.exe" disabled={galleryDlBusy} /><Button variant="outline" size="sm" onClick={chooseGalleryDl} disabled={galleryDlBusy}>选择文件</Button><Button size="sm" onClick={saveGalleryDlPath} disabled={galleryDlBusy || !galleryDlPath.trim()}>校验并保存</Button></div>{galleryDlMessage && <p className="settings-message" role="status">{galleryDlMessage}</p>}</div>
        </CardContent></Card>
        ...
        <Card id="extension-settings" tabIndex="-1"><CardHeader><CardTitle>浏览器 Extension</CardTitle><CardDescription>Full Package 会预置 Extension；Core Package 请从 GitHub 下载后按浏览器指南加载。</CardDescription></CardHeader><CardContent>
          <StatusRow icon="browser" label={extension.files_ready ? "Extension 文件已就绪" : "未找到完整 Extension"} detail={extension.message} ready={extension.files_ready} />

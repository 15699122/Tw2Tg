...
- Privacy: account names, Tweet IDs, pairing tokens, and Extension identity are omitted. The token is masked in screenshots and must not be copied into docs, logs, or diagnostic output.
...
- Security: one submitted Desktop screenshot displayed the pairing token in plaintext. The token value is omitted from all records. Rotate it by restarting Desktop when no jobs are active, then update the Extension's stored token. Future evidence must show only masked tokens and boolean auth outcome/error code.
...
- This is stronger stage evidence than the prior all-zero snapshot: WebSocket connections reach Desktop's accept loop, but no authentication envelope is counted. 99 connections closed before auth; the 100th was not yet counted closed in the screenshot. A bad token cannot explain `auth_received=0`, because the server checks token equality only after receiving/parsing the auth envelope and would increment `auth_failed` for a mismatch. HTTP 101 remains handshake-only evidence.
- The earlier six DevTools rows had 2–52 ms durations, suggesting early teardown rather than the listener's 5-second auth read timeout, but they were not correlated to the refreshed cumulative counters. Current failure boundary is before Desktop receives authentication; the Extension event/send lifecycle and early connection close still need discrimination. No response-path or token defect is established.
...
- Security: the newest Desktop screenshot again exposed the pairing token in plaintext. The token is not retained in this record; rotate it by restarting Desktop with no active jobs, then replace the stored Extension value. Future evidence must mask tokens and omit full frame payloads.
- Next observation: initiate one reconnect after token rotation; capture before/after counter deltas and the selected request's Messages view. Report only whether `authenticate` was sent, the server counters, request duration, and close initiator if DevTools exposes it. If `authenticate` appears but `auth_received` remains zero, correlate that single request to the running Desktop process/session.

...
- Package metadata tag `v0.0.0-pre.1` is a local validation label, not a release tag. The output is a portable package directory; no `.7z` archive was generated because 7-Zip is unavailable. Signing/publishing was not attempted.
...
- aria2 is absent from this Full package because the package builder's optional source `sidecar/aria2` does not exist. A local aria2 1.37.0 binary is available at `E:\Shiraishi\VSCode Workspace\Tw2Tg\aria2\1.37.0\aria2-1.37.0-win-64bit-build1\aria2c.exe` and can be selected from Desktop settings for a separate transfer test; do not infer task/download success from the independent aria2 binary test.
- Browser Extension load, Desktop GUI launch, WebSocket authentication, request/response, download, and lifecycle manual checks remain `NOT RUN` or `BLOCKED` according to the queue. The build only removes package construction as a prerequisite; it does not resolve the prior user-reported no-response failure.

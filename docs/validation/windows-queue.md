...
| ID | 类别 | 验证项目 | 关联修改 | Windows 原因 | 前置条件 | 精确行为 | 预期结果 | 优先级 | 阻塞 Linux | 状态 |
|---|---|---|---|---|---|---|---|---|---|---|
| WQ-WS-01 | Browser/Runtime | Edge/Chrome Extension load and WebSocket permission | `extension/manifest.json`、popup/options、WebSocket bridge | 浏览器 MV3 service worker、host permission、真实 WebSocket API 和版本行为需实机 | Windows Edge/Chrome 116+、当前 Extension 目录、Desktop release | 分别加载 Extension；打开 popup/options；检查 manifest 无错误、Service Worker 正常、popup 可打开设置、options 可保存设置 | Extension 可加载；GUI 可操作；无 CSP/permission/import 错误；不能因静态 Node 测试替代 | P0 | no | Current Full package Extension Options page load/display `PASS` (user screenshot); complete permissions, service-worker health and popup assertions `BLOCKED`/`NOT RUN` |
| WQ-WS-02 | Security/Pairing | Authenticated loopback WebSocket | `websocket_transport.rs`、`websocket-bridge.js`、Desktop `get_extension_status` | token 配对、错误 token、未认证请求和本地 token 展示需目标环境 | Desktop release、Edge/Chrome、受控窗口 | 复制 Desktop 设置中的端口/token 到 options；先测试错误 token，再测试正确 token；发送 `query_status`/`archive_request`；重启 Desktop 重复配对 | 错误 token 明确认证失败且不调用业务 adapter；正确 token 后请求成功；token 不进入 Browser payload、Job spec 或日志；端口变化可诊断 | P0 | no | `FAIL` (latest user screenshots): Desktop counters show accepted 100, auth_received/succeeded/failed/response_failed all 0, close_before_auth 99, close_after_auth 0. Edge reports `ERR_SOCKET_NOT_CONNECTED`; Popup remains disconnected/Native fallback. No auth message was received, so a wrong-token diagnosis is not supported. See latest queue/history follow-up for the next discriminator. |
| WQ-WS-03 | Lifecycle | Worker/Desktop reconnect and pending cleanup | WebSocket retry、Native fallback、RuntimeState executor replacement | MV3 worker 休眠/重启、Desktop 重启和旧 executor 代际需实机 | WQ-WS-01/02、受控 X fixture | 请求 pending 时停止/重启 Desktop、重载 Service Worker、断开/恢复网络；观察 popup/options 状态；再次提交新 request_id | pending 明确失败且不串线；worker 重启从 storage 恢复并重新认证；重连有界；旧 executor 不接收新请求；Native fallback 状态明确 | P0 | no | Current handoff `NOT RUN`; live reconnect/pending sequence `BLOCKED` — `COMPUTER_USE_UNAVAILABLE`; no current GUI sequence executed |
| WQ-WS-04 | GUI/Accessibility | Popup/options visual and keyboard behavior | `extension/popup.*`、`extension/options.*`、截图参考样式 | WebView/browser chrome、DPI、键盘和焦点需 Windows 浏览器验证 | WQ-WS-01、Edge/Chrome | 100%/125%/150% 缩放检查 popup/options；测试 Tab、Enter、Space、Escape/关闭、长状态文案和设置保存失败 | 状态层级清晰；无溢出/遮挡；焦点可见；保存失败保留输入；显示未配置/连接中/已认证/认证失败/回退等真实状态 | P1 | no | Prior-revision basic UI display `PASS` (limited, user-reported); current-revision visual states/scaling/keyboard/focus/error states `BLOCKED` — `COMPUTER_USE_UNAVAILABLE` |
| WQ-WS-05 | Packaging | Extension ZIP/Full package includes GUI and WebSocket bridge | `release-assets.mjs`、`native-host-package.mjs`、manifest/package scripts | 真实 Windows artifact、文件编码、Extension ID 和发布 zip 只能由 Windows/CI 验证 | 固定 Extension ID、release tag、Windows runner | 生成 Extension ZIP/Full package；检查 popup/options/src bridge 文件、版本、hash、无 tests/secrets；加载 ZIP | Extension ZIP (12-file inventory) and current Full package assembly/inventory `PASS`; Full package Dashboard readiness `PASS` (3/3); Full package bundled Extension-directory load `PASS` (user report); ZIP load `PASS` (user report) | P0 | no | At `fcde594`: Full package directory assembly and Extension ZIP build/extract/inventory `PASS` (12/12); Extension identity parity `PASS`; Bandizip `.7z` create/test/extract/hash compare `PASS` (77 files). Corrected Full package Sidecar running state and bundled Extension Options display `PASS` (user screenshots); ZIP load `NOT RUN`; signing/release gate `NOT RUN`. aria2 was manually staged from the user's local source directory into this validation artifact; release-builder behavior is unchanged. |
...
2. 启动 Desktop，在设置页确认 WebSocket 端口；配对 token 只在当前会话中安全传递，不记录到文档、截图或日志。因之前截图暴露了 token，下一轮应先在无活动任务时重启 Desktop 生成新 token，再将新 token 粘贴到 Extension options。确认 options 能保存并恢复设置。
...
2. Start `xarchive-desktop.exe`. If the first-use “选择下载目录” panel appears, choose either “创建便携目录” or “使用系统下载目录” and wait for setup to finish. On the Dashboard, click “启动 Sidecar” and wait until the status reads “Sidecar 正在运行” / `ready` (the app performs the v2 `hello → ready` handshake). Do not submit an archive task while it still reads “Sidecar 未启动”. If start fails, save the complete red error text and check `<package root>\logs\xarchive-*.log` and whether `xarchive-downloader.exe` remains running; redact tokens and URLs from evidence. Then open Settings → Extension and confirm WebSocket is enabled on port `17321`; use the app's copy action for the current token. Keep the token private and do not include it in screenshots, logs, or notes.
...
- The submitted Desktop screenshot visibly exposed the pairing token. Treat it as compromised: when there are no active jobs, restart Desktop to rotate its process-generated token, then replace the Extension's stored token with the new one. Do not copy the exposed value into logs or documentation.
...
- This narrows the failure stage: the Desktop listener accepted the WebSocket connections, but none of the 100 reached the server's authentication-message receive counter; 99 closed before authentication. A wrong token would be evaluated only after `auth_received` increments and would increase `auth_failed`; therefore the current evidence does not support “wrong token” as the immediate cause. HTTP 101 still proves only the Upgrade handshake. Earlier six Network rows had short durations (2–52 ms), consistent with early closure rather than the server's 5-second authentication read timeout, but those rows are not correlated to these cumulative counters.
- Root cause remains unresolved between Extension WebSocket event/send lifecycle and connection teardown/timing. Do not attribute the failure to token contents or claim the auth-response path failed: the server has not recorded receiving an auth message. The prior all-zero diagnostic screenshot is superseded by this refreshed counter snapshot.
- Next minimal capture: restart Desktop only when no jobs are active to rotate the exposed token; replace it in Extension Options; initiate exactly one reconnect; immediately note the counter deltas. Select that exact Network request and inspect Messages for an outbound `authenticate` frame. If absent, capture only the Service Worker console error/state; if present while `auth_received` stays 0, record request duration and whether the client or server initiates close. Never show the token or full frame payload.
- The newer Desktop screenshot again exposes the pairing token. Its value remains omitted; rotate it before any further pairing attempt and keep future screenshots masked.

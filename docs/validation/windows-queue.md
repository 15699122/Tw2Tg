...
- Current WQ-WS-05 Full package directory assembly and Extension ZIP plan/build/extract/inventory: `PASS` (completed at `fcde594`, 12/12 files). Browser load remains `NOT RUN`; only the Windows Tauri executable/package assembly ran. Prior-revision browser-load results remain historical evidence. Full `.7z`/signing/release gate: `NOT RUN`.
...
- The package uses local validation tag `v0.0.0-pre.1`; this is not a release artifact. The Full package is an assembled portable directory, not a `.7z` archive (7-Zip is not installed). Browser load, GUI startup and live WebSocket pairing remain `NOT RUN`/`BLOCKED` as separately recorded.
- The existing local aria2 binary is not bundled: the package builder reads optional binaries from `sidecar/aria2`, which is absent. The separate binary at `aria2/1.37.0/aria2-1.37.0-win-64bit-build1/aria2c.exe` may be selected in Desktop settings if a download-specific check is attempted. Independent aria2 tests do not establish Desktop archive/download success.
- Full package checks: `package_type=full`, `platform=windows-x64`, desktop executable, Extension source folder, Native Host executable/manifest, worker and Full gallery-dl payloads present; `package-manifest.json` Extension ID and Native Host origin agree with the Extension manifest. aria2 absent (optional component).
...
1. In Explorer, open `E:\Shiraishi\VSCode Workspace\Tw2Tg\validation-artifacts\windows-ws-fcde594\full-package`. Confirm `xarchive-desktop.exe`, `package-manifest.json`, `extension`, `native-host`, and `sidecar` are present. Do not move the folder after installing/registering Native Host because its manifest points to the executable's absolute path.
2. Start `xarchive-desktop.exe`. Confirm the Dashboard renders, then open Settings → Extension. Confirm WebSocket is enabled on port `17321`; use the app's copy action for the current token. Keep the token private and do not include it in screenshots, logs, or notes.
3. In Edge, visit `edge://extensions`, turn on Developer mode, and load the unpacked Extension directory at `...\full-package\extension`. If testing the ZIP instead, extract `XArchive-v0.0.0-pre.1-extension.zip` to a separate local directory first, then use “Load unpacked” on that extracted folder. Test one installation at a time because both copies share one Extension ID.
...
9. If additionally verifying a real download, the Full package currently has no bundled aria2. Select `E:\Shiraishi\VSCode Workspace\Tw2Tg\aria2\1.37.0\aria2-1.37.0-win-64bit-build1\aria2c.exe` in the app's aria2 settings and save/validate the path first. Use only a controlled test item; task creation alone is not a download PASS. Confirm a completed file and content/hash before claiming success.
10. Record each item separately as `PASS`, `FAIL`, `BLOCKED`, or `NOT RUN`, together with the package/source revision `fcde594`, browser version and redacted evidence. Signing/release gate remains out of scope.

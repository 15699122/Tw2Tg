...
| MANUAL-WIN-FULL-SIDECAR-02 | `BLOCKED` (corrected package GUI retest) | The supplied package failed because its frozen worker rejected Desktop's timeout arguments. Corrected worker protocol probe and corrected archive checks pass. Launch the corrected package at `validation-artifacts/windows-ws-fcde594/full-package-sidecar-fix`, finish first-run folder setup, then click “启动 Sidecar” and verify `ready`. Computer Use was stopped by the user with physical Escape, so no GUI result is available yet. |
...
- Status: supplied Full package Sidecar launch `FAIL` (root cause confirmed); fresh worker exact-argument handshake `PASS`; corrected archive test and extraction/hash comparison `PASS`; corrected package Desktop GUI launch `BLOCKED` because Computer Use was stopped by the user with physical Escape. No GUI result is inferred from the process-level probe.
- Manual retest: extract the corrected `.7z` to a new local folder (or use the assembled `full-package-sidecar-fix` directory), complete “选择下载目录”, click “启动 Sidecar”, and wait for `Sidecar 正在运行` / `ready`. Only then submit one controlled task. If it fails, capture the full alert and newest `<package root>\\logs\\xarchive-*.log`, plus `Get-Process xarchive-downloader -ErrorAction SilentlyContinue | Select-Object Id,ProcessName,Path`; redact credentials, tokens, and private URLs.

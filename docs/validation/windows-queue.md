...
| WQ-WS-02 | Security/Pairing | Authenticated loopback WebSocket | `websocket_transport.rs`、`websocket-bridge.js`、Desktop `get_extension_status` | token 配对、错误 token、未认证请求和本地 token 展示需目标环境 | Desktop release、Edge/Chrome、受控窗口 | 复制 Desktop 设置中的端口/token 到 options；先测试错误 token，再测试正确 token；发送 `query_status`/`archive_request`；重启 Desktop 重复配对 | 错误 token 明确认证失败且不调用业务 adapter；正确 token 后请求成功；token 不进入 Browser payload、Job spec 或日志；端口变化可诊断 | P0 | no | `FAIL` at `fcde594` (user screenshots): `accepted=100, auth_received=0, close_before_auth=99`, `ERR_SOCKET_NOT_CONNECTED`. Root cause identified in shared code and fixed in this batch: the accepted stream inherited the listener's non-blocking mode on Winsock, so the authentication read returned `WouldBlock` and was counted as a pre-auth close. Linux-verified with a defect-sensitive regression test. **Status: revalidate against the new handoff** — refresh Desktop counters, perform one reconnect, and compare `handshake_failed`/`auth_read_failed`/`auth_received` before and after. Token must not appear in evidence. |
...
| WQ-WS-05 | Packaging | Extension ZIP/Full package includes GUI and WebSocket bridge | `release-assets.mjs`、`native-host-package.mjs`、manifest/package scripts | 真实 Windows artifact、文件编码、Extension ID 和发布 zip 只能由 Windows/CI 验证 | 固定 Extension ID、release tag、Windows runner | 生成 Extension ZIP/Full package；检查 popup/options/src bridge 文件、版本、hash、包内测试文件与构建产物；Token 隐私/安全检查按用户要求跳过 | Extension ZIP (12-file inventory) and current Full package assembly/inventory `PASS`; Full package Dashboard readiness `PASS` (3/3); Full package bundled Extension-directory load `PASS` (user report); ZIP load `PASS` (user report) | P0 | no | At `fcde594`: Full package directory assembly and Extension ZIP build/extract/inventory `PASS` (12/12); Extension identity parity `PASS`; Bandizip `.7z` create/test/extract/hash compare `PASS` (77 files). Corrected Full package Sidecar running state and bundled Extension Options display `PASS` (user screenshots); ZIP load `NOT RUN`; signing/release gate `NOT RUN`. aria2 was manually staged from the user's local source directory into this validation artifact; release-builder behavior is unchanged. Pairing-token privacy/security check `NOT APPLICABLE` per user instruction. |

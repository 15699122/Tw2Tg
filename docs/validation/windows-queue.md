...
1. Choose one package location. For the already assembled directory, open `E:\Shiraishi\VSCode Workspace\Tw2Tg\validation-artifacts\windows-ws-fcde594\full-package`. To manually test the `.7z`, use Bandizip to extract `E:\Shiraishi\VSCode Workspace\Tw2Tg\validation-artifacts\windows-ws-fcde594\XArchive-v0.0.0-pre.1-windows-x64-full.7z` into a new test folder; the archive expands its contents directly under that folder. The `.7z` has already passed integrity testing and a 77-file SHA-256 comparison. Confirm `xarchive-desktop.exe`, `package-manifest.json`, `extension`, `native-host`, and `sidecar` exist at the chosen package root. Prefer the assembled directory for the first run because the bundled Native Host manifest contains the original executable path; if running from a separately extracted location and Native Messaging is under test, repair/re-register Native Host from that location before testing. Do not move a package after registering Native Host.

...
|---|---|---|
| MANUAL-WIN-E5-01 | `BLOCKED` — `COMPUTER_USE_UNAVAILABLE` | Launch `validation-artifacts/windows-full-e5-e7-current-source-20260923/xarchive-desktop.exe`; confirm XArchive renders and closes; confirm the process listens on `\\.\pipe\xarchive-v1`; test two sequential Native Host requests through the packaged executable, then Desktop close and pipe cleanup. Verify another Windows user cannot open the pipe. Save screenshot, process/pipe evidence, and logs. |

...
| `desktop/src-tauri/src/commands.rs` | App status（包含 database、Sidecar 和 executor 生命周期状态）、Sidecar 生命周期、Job 查询、executor submit/query/cancel/shutdown control commands、archive root、文件夹打开和 runtime health commands；包含 Sidecar 配置解析与退出状态刷新 | 保持现有 command 名称、参数和 RuntimeState 锁语义；executor control commands 只操作 Job control/persistence，不执行真实归档 I/O；`executor` status 只反映 worker ownership |
| `desktop/src-tauri/src/archive.rs` | `archive_tweet` command、`ArchiveJobSubmissionAdapter` fallback 对照、`ArchiveExecutionContext` resource bundle、`ArchiveExecutionJob` execution-port adapter、Job 创建/重复任务处理、DownloadRouter 调用、Sidecar archive/download request/result、Browser user/relationship merge、归档提交、Job 事件/失败状态和安全错误映射 | 保持 metadata identity binding、文件事件归一化、错误脱敏和现有 RuntimeState 长锁语义；`ArchiveExecutionJob` 已实现 identity/download/commit/event mapping，但当前仍由同步 fallback 间接使用 context，后台 worker 接管另行进行 |
| `desktop/src-tauri/src/executor.rs` | R1 Job executor command/state model；`ExecutorRuntime`、`ArchiveApplicationService`、`JobExecutorHandle`、`JobPersistence`/`JobExecution` ports、`JobDatabaseFactory`、`CommitRecoveryFactsProvider`/`InMemoryCommitRecoveryFactsProvider`、`CommitRecoveryBatchResult`、`ArchiveJobSubmissionAdapter`、`JobExecutionResult`/`JobExecutionError`、`JobSummary`→`JobSnapshot` 字段投影、纯 Rust 恢复决策与动作、in-memory/SQLite adapter、`ExecutorEvent` lifecycle model、核心 `JobEvent` 映射、事务性状态事件去重、recovery/cancel/shutdown/completion/execute contract、bounded channel、submit/duplicate/query/start/cancel/shutdown/event ordering 和单 worker invariant 测试 | `ExecutorRuntime` 已由 `RuntimeState` 持有；Tauri control commands 通过独立 persistence context 完成 submit/query/cancel/shutdown boundary，`execute_persisted` 已连接 fake execution port 与 persistence/event lifecycle，但真实 `ArchiveExecutionContext` 尚未由 worker 消费；真实 FileStore、Sidecar、completion/recovery worker I/O 仍未接入 |

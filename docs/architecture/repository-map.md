...
| `docs/releases/v0.2.0-pre.9.md` | Windows WebDriver 驱动探测修复（服务 banner 补丁 + `postinstall`/`pretest:e2e*` hooks）、WebDriver 工具链规则修正、readiness 诊断与发布门禁接线 | 必须以 `v0.2.0-pre.9` tag、对应 source SHA、GitHub Actions run 和实际 Release assets 为准；Windows hosted-runner session、Registry、Named Pipe、浏览器集成和真实 runtime 状态不得写成 PASS |

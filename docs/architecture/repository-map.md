...
| `desktop/scripts/wdio-tauri-service.mjs` | WDIO worker 适配层；复用官方 launcher，跳过依赖 `plugin:wdio` 的单窗口 focus probe，并避免 service 与 spec 重复清理 mock/session | 不改变普通 artifact 的 capability/guest JS 边界；高级 spec 负责显式 mock restore；Windows session/driver 自动回收仍需实机验证；不解析或替换已安装 service 的 Edge driver banner |

...
| `desktop/scripts/run-wdio-advanced.mjs` | 跨平台启动高级 WDIO E2E，设置 `WDIO_ADVANCED`/`WDIO_CAPTURE_LOGS` 并在 Windows 使用 `wdio.cmd` | 不执行测试逻辑；保持 Windows PowerShell/CMD 与 Linux 命令行为一致 |

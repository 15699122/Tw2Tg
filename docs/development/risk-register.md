...
| RISK-017 | 依赖审计命中 RUSTSEC-2026-0285 与 Node 测试链公告 | P1 | OPEN | `Cargo.lock`、`package-lock.json` | 发布构建改用 `cargo --locked`；worker workflow 固定 `pyinstaller==6.22.3`（已用 `pip index versions` 确认版本存在）。rustls 与 Node 依赖链升级待单独评估 | `cargo audit`、`npm audit` 复跑 |

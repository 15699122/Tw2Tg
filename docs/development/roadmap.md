...
当前进度：已完成 Windows-safe 用户目录名生成、基于用户名/文本/Tweet 类型/媒体类型的确定性 TagEngine 规则匹配，以及 SQLite users/user_names/tags/tweet_tags Repository API。Quote/Reply 建模已完成跨层实现（2026-09-12）：`xarchive-protocol` `BrowserTweet` 支持嵌套 `quoted_tweet` 与 `reply_to` 及递归校验，shared JSON Schema 同步；Extension content script 从 DOM quote card 提取嵌套引用；`xarchive-storage` 通过 migration `0003_quote_reply_relationships.sql` 为 `tweets` 增加 `reply_to_tweet_id`/`quoted_tweet_id` 列，`update_tweet_metadata` 从 `ArchiveMetadata`（含 `ArchiveQuotedTweet`）持久化关系，`tweet_relationships` 查询关系；Desktop `archive_tweet` 通过 `merge_browser_relationships` 将浏览器 DOM 提供的 reply/quote 关系填入 Sidecar 元数据空缺（不覆盖 Sidecar 已有数据，`post` 类型可被 DOM 证据升级为 reply/quote）。profile 文件已完成（2026-09-12）：`xarchive-storage` 新增 `UserProfileSnapshot`/`UserProfileFile` 类型、`Database::user_profile()` 快照查询（含最新名称与完整名称历史）、`FileStore::write_user_profile()` 写入 `Users/<stable>/profile.json`，`ArchiveService::refresh_author_profile()` 在 `complete_local_archive` 中自动注册归档作者、刷新 profile 文件（`user_id` 缺失时静默跳过），`upsert_user` 增加名称去重（username 与 display_name 均未变化时不重复记录）。新增测试：`persists_user_name_history_and_stable_directory_name` 名称去重断言、`completes_local_archive_and_writes_portable_metadata` 扩展为 profile.json 内容与 tweet-user 关联断言（storage 测试 19/19）。Windows 已对同一 working tree 完成三轮自动化复验——首轮 87/87（2026-09-11/12）；次轮 88/88（2026-09-12，含 profile 针对性测试 3/3）；三轮 88/88（2026-09-12 13:34–13:42，含 M5 Quote/Reply + Profile 定向回归 6/6，Node check/test/build 7/7，Sidecar pytest 10/10，Tauri Release build 与 Debug startup 均 PASS）；Computer Use 浏览器只读 AX 探测 PASS（Edge tab 检测可用），但原生 Windows 桌面 Computer Use 服务 `sky.list_apps()` 返回 `Trusted RPC service is not configured`，无法进行 Tauri 原生窗口交互验收；默认 Windows 命令环境 `PYTHON` 为空导致 2 项 supervisor 真实 worker 测试 `NotRunning`，属于环境前置条件 FAIL，设置项目 `.venv` 后完整 88/88 通过，当前没有可归因于 M5 业务代码的 Windows 自动化 FAIL。真实浏览器 DOM / gallery-dl 实际 payload 的端到端验证、Desktop 应用级旧库 0001→0003 升级和 Unicode 路径写入仍为 `WINDOWS_VERIFICATION_PENDING`/`NOT RUN`（WQ-M5-05~08、WQ-M5-10~11）。

...
当前进度：已完成 `xarchive-download` 协议模型、RPC 请求构造、状态/字节数解析、安全校验和 fixture 测试；Windows Rust check/test/format 已通过，`large_enum_variant` 已修复，待 Windows clippy 复验；真实 aria2c Supervisor、HTTP/WebSocket client、断点恢复和 Windows 打包仍待实现。默认下载仍使用 gallery-dl。

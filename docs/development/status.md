...
- **当前 Git 事实（2026-09-20）：**当前分支为 `feature/u7-desktop-production-integration`，HEAD 为 `97959fa974b32f3a357a0d80682e6aefefa8533e`（`feat: harden browser extension flow`）；working tree dirty，仅包含 `v0.2.0-pre.5`/`v0.2.0-pre.6` Release Notes 中文化和相关发布文档修改，分支基线与 `origin/feature/u7-desktop-production-integration` 一致；`v0.2.0-pre.6` tag 当前仍指向上一发布 commit，待本轮 Release Notes 修正提交后同步更新。此前记录的 `bd3e58d` dirty-source 快照属于历史验证上下文，不代表当前 source；Linux source 仍是唯一事实来源。

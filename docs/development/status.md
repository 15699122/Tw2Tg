...
- **当前 Git 事实：**工作区当前位于 `feat/extraction-aria2-pipeline`，HEAD 为 `59d3263`；本节不采用其他工作区或用户输入中的历史分支/提交作为当前事实。工作区仍包含上一轮 U2 Sidecar cancellation 的未提交代码、测试和文档修改。
- **CURRENT / LINUX_VERIFIED：**Python worker 已具备下载期间的 cooperative `cancel`/`shutdown`、超时、EOF 退出和 POSIX 子进程 session；Rust Sidecar Supervisor 现在也为 Sidecar 建立独立 Unix process group，并在 shutdown/force cleanup 时回收整个进程组；Sidecar pytest 当前为 17/17，Supervisor 专项测试 4/4，Rust/Node 适用验证已通过。
- **MIGRATION：**Sidecar protocol v1、`download` command、gallery-dl 媒体下载、`file/progress/complete` events、`DownloadRouter` fallback 和 `archive_tweet` 仍是运行时链路。它们不能被描述为已删除能力。
- **U3 contract（LINUX_VERIFIED）：**Sidecar protocol v2 typed contract 已落地：`xarchive-protocol` 新增 `sidecar_v2` 模块（`hello` capability handshake、`extract` command、`ready/extraction_started/extracted/cancelled/failed/log` 事件、typed `ExtractionResult`、Tweet ID 与 X URL 绑定、header allowlist、v1/unknown field/缺失 capability 显式拒绝），Python 侧新增 `protocol_v2`/`worker_v2`/`extraction` 模块与 contract tests，Schema 与 fixtures 新增 `sidecar-v2-command/event` 和 v1 rejection fixture。Rust protocol 测试 15/15 通过。Supervisor 尚未 spawn v2 worker，Desktop 尚未消费 v2 事件；运行时接线属于 U7，当前运行链路仍是 v1。
- **U4 extraction-only adapter（LINUX_VERIFIED，v2 路径）：**`extraction.py` 已收缩为 extraction-only：gallery-dl 命令强制 `--skip-download` 且防御性拒绝 `--directory`/`--filename`/`--download`；metadata 读取兼容 `info.json` 与 `*.info.json`；媒体项生成 stable media identity（metadata `media_id` 优先，其次 URL 文件名段，最后 `media-NN` 顺序 fallback）；filename 经净化（分隔符/控制字符/`..`/长度/隐藏名）或退回 `NN.bin`；result 只承载 metadata + media plan，`ExtractionResult` 已彻底移除 `raw` 字段，不携带任何已下载文件事实或 raw metadata；header 仅 Referer/Accept 且过 secret 检查。新增 `test_extraction_only.py`（5 tests）与 `test_protocol_v2.py` U4 tests（fixed schema keys、fake gallery-dl 写媒体但 result 无下载事实、AUTH_REQUIRED、filename 净化、stable identity）。Sidecar pytest 33/33 通过。该路径尚未被 Supervisor spawn、Desktop 消费；v1 媒体下载链路仍是 MIGRATION 残留。
- **U5 aria2-only transfer driver（LINUX_VERIFIED，driver/plan 层）：**`xarchive-download` 已新增从 typed `ExtractionResult` 构建 `MediaTransferPlan` 的纯逻辑层，以及与 Tauri/SQLite/Sidecar 解耦的 aria2-only `Aria2TransferDriver`。已覆盖 allowlisted headers、stable media identity、safe filename/URL 校验、multi-GID polling、plan-order completion、progress regression、timeout、cancel、shutdown、error/removed classification、提交失败和 partial/`.aria2` cleanup。`cargo test -p xarchive-download` 20/20、driver integration 7/7、workspace test、workspace clippy、fmt 通过。该 driver 尚未接入 Supervisor/Desktop executor；v1 `DownloadRouter` fallback 仍保留为 MIGRATION 残留。
- **U6 URL refresh contract（LINUX_VERIFIED，driver/plan 层）：**`xarchive-download` 已新增 401/403/expired/signature/access-denied 分类与 `RefreshCoordinator`；只对 URL 过期类失败执行一次完整 extraction refresh，refresh 后按 stable media identity + filename 集合匹配，集合变化返回 `EXTRACTION_RESULT_CHANGED`，普通 transfer failure、cancel、shutdown、timeout、权限/磁盘类错误不 refresh。`xarchive-download` unit 23/23、driver integration 7/7、Sidecar pytest 33/33、Desktop Node 33/33、Extension 7/7、workspace test/clippy/fmt 通过。该 contract 尚未接入 Supervisor/Desktop executor；U7 production wiring 和 U8 old-path removal 仍未完成。
- **PLANNED：**ComponentManager、Core Bootstrap、Release assets、Native Host/Extension 安装流和 Offline Bundle 尚未完成；Sidecar v2 Supervisor spawn、Desktop consumer、aria2/refresh production wiring 也未完成。
- **WINDOWS_VERIFICATION_PENDING：**Windows Job Object/process-tree、文件锁、packaged worker、真实 shutdown 时序、WebView2、Named Pipe、Registry、真实账号和发布资产仍需集中验证。当前 Windows 路径仍未声称使用 Job Object；Linux 侧只完成了 Unix process-group contract。

...
- **当前 Git 事实（2026-09-20）：**当前分支为 `feature/u7-desktop-production-integration`，HEAD 为 `ea2b8d3afb289239edec29e2e00620870bed2fe6`（`release: prepare v0.2.0-pre.5`）；working tree dirty，包含本轮 E1/E2 Extension、Browser protocol、Desktop transport、Schema 和文档修改，且分支基线与 `origin/feature/u7-desktop-production-integration` 一致。此前记录的 `bd3e58d` dirty-source 快照属于历史验证上下文，不代表当前 source；Linux source 仍是唯一事实来源。

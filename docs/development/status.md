...
- **当前 Git 事实（2026-09-20）：**当前分支为 `feature/u7-desktop-production-integration`，HEAD 为 `435a9085a7d66bb12b9b515012999730080573d6`（`docs: localize pre-release notes`）；working tree clean，分支基线与 `origin/feature/u7-desktop-production-integration` 一致；`v0.2.0-pre.6` tag 已同步指向当前 HEAD。此前记录的 `bd3e58d` dirty-source 快照属于历史验证上下文，不代表当前 source；Linux source 仍是唯一事实来源。
